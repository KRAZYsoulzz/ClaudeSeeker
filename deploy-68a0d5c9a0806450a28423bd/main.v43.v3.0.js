...
const state = { date: (window.etLocalISO ? etLocalISO() : (new Date()).toLocaleDateString('en-CA')), items: [] }; window.__appState = state;;
...
window.appLoadTicketForDate = function(ymd, ticket){
  try{
    if (window.__appState){
      __appState.date = ymd;
      __appState.items = Array.isArray(ticket.items) ? ticket.items.map(it=>({ ...it, addons: Array.isArray(it.addons)? it.addons.map(a=>({...a})) : [] })) : [];
      if (__appState.totals && ticket.totals){ __appState.totals = { ...ticket.totals }; }
    }
    if (typeof renderTicket === 'function') renderTicket();
    if (typeof persist === 'function') try{ persist(); }catch(e){}
    if (typeof syncTodayToHistory === 'function') try{ syncTodayToHistory(); }catch(e){}
  }catch(e){}
};
...
(function(){
  ...
  function localISO(d){ d = d? new Date(d): new Date(); return [d.getFullYear(), String(d.getMonth()+1).padStart(2,'0'), String(d.getDate()).padStart(2,'0')].join('-'); }

  function resetTodayTicket(today){
  try{
    var empty = {
      date: today,
      items: [],
    ...
    try{ localStorage.setItem('et_ticket_v14', JSON.stringify(empty)); }catch(_){}
    try{ if (typeof renderTicket === 'function') renderTicket(); }catch(_){}
    try{ if (typeof persist === 'function') persist(); }catch(_){}
  }catch(_){}
}
function ensureDayConsistency(){
    try{
      var today = localISO();
      var ticket = lsGet('et_ticket_v14');
      var lastReset = lsGet('et_lastResetDate');
      var prev = lsGet('et_prevTicketBackup');

      if (prev && prev.date === today){
        if (!ticket || ticket.date !== today){
          lsSet('et_ticket_v14', prev.ticket || null);
        }
        lsSet('et_lastResetDate', today);
        return;
      }
      ...
      if (ticket && ticket.date !== today){
        ...
        lsSet('et_prevTicketBackup', { date: ticket.date || null, ticket: ticket });

        lsSet('et_lastResetDate', today);

        try{ resetTodayTicket(today); }catch(_){}

        setTimeout(function(){ try{ if (typeof renderTicket !== 'function') location.reload(); }catch(_){ location.reload(); } }, 20);
        return;
      }
    }catch(_){}
  }
  ...
  document.addEventListener('visibilitychange', function(){
    if (document.visibilityState === 'visible') ensureDayConsistency();
  });
  if (document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', ensureDayConsistency);
  } else {
...
(function(){
  ...
  function lsSet(k,v){ try{ localStorage.setItem(k, JSON.stringify(v)); }catch(_){ } }
  function show(){
    var b=document.getElementById('changelogBackdrop'), m=document.getElementById('changelogModal');
    if(!b||!m) return; b.style.display='block'; m.style.display='block';
  }
  function hide(){
    var b=document.getElementById('changelogBackdrop'), m=document.getElementById('changelogModal');
    if(!b||!m) return; b.style.display='none'; m.style.display='none';
  }
  function wire(){
    var ok=document.getElementById('clOk'), x=document.getElementById('clCloseX');
    if(ok) ok.addEventListener('click', function(){ lsSet('et_lastSeenVersion', window.APP_VERSION); hide(); });
    if(x)  x.addEventListener('click', function(){ lsSet('et_lastSeenVersion', window.APP_VERSION); hide(); });
  }
  function maybeShow(){
    var seen = lsGet('et_lastSeenVersion');
    if (seen !== window.APP_VERSION) { show(); }
  }
  if (document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', function(){ wire(); maybeShow(); });
  } else {
    wire(); maybeShow();
  }
})();

function checkAndRolloverTicket(){
  try{
    const today = (typeof localISO==='function') ? localISO() : new Date().toISOString().slice(0,10);
    let cur = null;
    try{ cur = JSON.parse(localStorage.getItem('et_ticket_v14')); }catch(_){}
    if (cur && cur.date && cur.date !== today){
      if (typeof upsertTicketIntoHistory === 'function') {
        try { upsertTicketIntoHistory(cur); } catch(e){}
      }
      try { localStorage.setItem('et_prevTicketBackup', JSON.stringify({ date: cur.date, ticket: cur })); } catch(e){}
      const emptyTicket = {
        date: today,
        items: [],
        addons: [],
      ...
      try { localStorage.setItem('et_ticket_v14', JSON.stringify(emptyTicket)); } catch(e){}
      if (typeof renderTodayCard === 'function') {
        try { renderTodayCard(emptyTicket); } catch(e){}
      }
    }
  }catch(e){
